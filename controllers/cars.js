...
const CarStyles = require('../models/styleCar');
...
const SSPATH = __dirname+'/../data/styles.json';
...
function createCsv(req, res){
	...
	var result = CSV.create(filename, datas);
...
function createStyle(req, res){
	...
	req.on('end', () => {
		...
		// save the data
		fs.writeFile(SSPATH, JSON.stringify(data), err => {
